<template>
    <div class="warehouse-in-container">
        ...
        <el-card class="header-card">
            <div slot="header" class="clearfix">
                ...
                <el-button style="float: right;" type="primary" icon="el-icon-plus"
                    @click="showCreateDialog">新建入库单</el-button>
            </div>
            ...
            <el-form :inline="true" size="small" :model="searchForm" class="search-form">
                ...
                <el-form-item label="供应商">
                    <el-select v-model="searchForm.supplier" placeholder="请选择或输入供应商" clearable filterable allow-create
        ...
        <el-card class="table-card">
            <el-table size="small" :data="tableData" border stripe style="width: 100%"
                @selection-change="handleSelectionChange">v-loading="loading" :data="tableData" border stripe
                style="width: 100%"
                ...
                <el-table-column prop="inboundType" label="入库类型" width="100" align="center">
                    <template slot-scope="scope">
                        <el-tag :type="getTypeColor(scope.row.inboundType)">{{ getTypeName(scope.row.inboundType)
                            }}</el-tag>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="200" fixed="right" align="center">
                    <template slot-scope="scope">
                        <el-button v-if="scope.row.status === 'pending'" size="mini" type="primary"
                            @click="confirmInbound(scope.row)">确认入库</el-button>
                ...
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="供应商" prop="supplier">
                            <el-select v-model="inboundForm.supplier" placeholder="请选择或输入供应商" style="width: 100%"
                        ...
                        <el-form-item label="入库物料">
                            ...
                            <el-table :data="inboundForm.materials" border style="margin-top: 10px;">
                                <el-table-column label="物料编码" width="150">
                                    <template slot-scope="scope">
                                        <el-select v-model="scope.row.materialCode" filterable allow-create
                                            default-first-option placeholder="选择或输入物料编码"
                                            @change="onMaterialChange(scope.$index, scope.row.materialCode)">
                                            <el-option v-for="material in materialOptions" :key="material.code"
                                ...
                                <el-table-column label="单位" width="100">
                                    <template slot-scope="scope">
                                        <el-select v-model="scope.row.unit" placeholder="选择或输入单位" filterable
                                ...
                                <el-table-column label="入库数量" width="120">
                                    <template slot-scope="scope">
                                        <el-input-number v-model="scope.row.quantity" :min="0" :precision="2"
                                            controls-position="right" style="width: 100%" placeholder="数量" />
                                    </template>
                                </el-table-column>
                                <el-table-column label="单价" width="120">
                                    <template slot-scope="scope">
                                        <el-input-number v-model="scope.row.unitPrice" :min="0" :precision="2"
                                            controls-position="right" style="width: 100%" placeholder="单价" />
                                    </template>
                                </el-table-column>
        ...
        <el-dialog title="入库单详情" :visible.sync="detailDialogVisible" width="70%">
            <div v-if="currentDetail">
                <el-descriptions :column="3" border>
                    ...
                    <el-descriptions-item label="入库类型">{{ getTypeName(currentDetail.inboundType)
                        }}</el-descriptions-item>
                    <el-descriptions-item label="状态">
                        <el-tag :type="getStatusColor(currentDetail.status)">{{ getStatusName(currentDetail.status)
                            }}</el-tag>
                    </el-descriptions-item>
        ...
        async saveInbound() {
            this.$refs.inboundForm.validate(async (valid) => {
                if (valid) {
                    ...
                    const invalidMaterials = this.inboundForm.materials.some(material =>
                        !material.materialCode || !material.materialName || !material.unit || material.quantity <= 0
                    )
