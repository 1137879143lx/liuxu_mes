...
// 添加编码生成锁，防止并发问题
const codeGenerationLocks = new Map()

class MaterialController {
  // 获取物料列表 - 修复版本
  async getMaterials(req, res) {
    try {
      const { page = 1, limit = 10, code, name, category, status } = req.query
  ...
  async updateMaterial(req, res) {
    try {
      ...
      // 只更新未删除的物料
      const material = await Material.findOneAndUpdate({ _id: id, isDeleted: { $ne: true } }, req.body, { new: true, runValidators: true })
  ...
  async generateMaterialCode(req, res) {
    try {
      ...
      // 使用分布式锁防止并发问题
      const lockKey = `code_gen_${todayPrefix}`

      // 等待获取锁
      while (codeGenerationLocks.has(lockKey)) {
        await new Promise((resolve) => setTimeout(resolve, 10))
      }

      // 设置锁
      codeGenerationLocks.set(lockKey, true)

      try {
        // 获取今天同前缀所有物料的编码（包括已删除和未删除的）
        ...
        console.log(
          `查找前缀 ${todayPrefix} 的所有物料:`,
          allMaterials.map((m) => m.code)
        )
        ...
        console.log(
          '已使用的序号集合:',
          Array.from(usedSequencesSet).sort((a, b) => a - b)
        )
        ...
        // 释放锁
        codeGenerationLocks.delete(lockKey)
      }
    } catch (error) {
  ...
  // 批量生成物料编码 - 新增方法
  async batchGenerateMaterialCodes(req, res) {
    try {
      const { category, categoryCode, count = 1 } = req.body
      ...
      // 使用分布式锁防止并发问题
      const lockKey = `batch_code_gen_${todayPrefix}`
      ...
      try {
        ...
        console.log(`批量生成成功，共生成 ${generatedCodes.length} 个编码`)

        res.json({
          code: 200,
          message: `批量生成成功，共生成 ${generatedCodes.length} 个编码`,
          data: {
  ...
  // 批量创建物料 - 新增方法
  async batchCreateMaterials(req, res) {
    try {
      ...
      for (let i = 0; i < materials.length; i++) {
        try {
          ...
          results.success++
          results.successData.push(material)
        } catch (error) {
          results.failed++
